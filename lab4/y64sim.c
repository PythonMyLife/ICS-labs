...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    switch (op){
      case A_ADD:
        if((val - argA) != argB){ovf=TRUE;}
        break;
      case A_SUB:
        if((val + argB) != argA){ovf=TRUE;}
        break;
      default:
        break;
    }
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if (need_imm == TRUE) {
        if( !get_long_val(sim->m,next_pc,&imm)) return STAT_ADR;
        next_pc += 8;
    }
    

    /* execute the instruction*/
    long_t argA,argB,argC;
    cond_t cond;
    switch (icode) {
      case I_HALT: /* 0:0 */
	return STAT_HLT;
	break;
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        ...
        argB = imm + get_reg_val(sim->r,regB);
        (void)set_long_val(sim->m,argB,argA);
        sim->pc = next_pc;
        break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        argA = imm + get_reg_val(sim->r,regB);
        if (!get_long_val(sim->m,argA,&argB)){
          err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,argA);
          return STAT_ADR;
        }
      ...
      case I_ALU: /* 6:x regA:regB */
        argA = get_reg_val(sim->r,regB);
        argB = get_reg_val(sim->r,regA);
        argC = compute_alu(ifun,argA,argB);
        set_reg_val(sim->r,regB,argC);
        sim->pc = next_pc;
        sim->cc = compute_cc(ifun,argA,argB,argC);
        break;
      case I_JMP: /* 7:x imm */
        next_pc = sim->pc;
        if (!get_byte_val(sim->m, next_pc, &codefun)) {
        ...
        else {sim->pc = sim->pc + 9;}
        break;
      case I_CALL: /* 8:x imm */
        regA = (regid_t)REG_RSP;
        argA = get_reg_val(sim->r,regA)-8;
        set_reg_val(sim->r,regA,argA);
        if ((argA<0) | (argA > sim->m->len)){
          err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,argA);
          return STAT_ADR;
        }
        set_long_val(sim->m,argA,next_pc);
        sim->pc = imm;
        break;
      case I_RET: /* 9:0 */
        regA = (regid_t)REG_RSP;
        argA = get_reg_val(sim->r,regA);
        (void)get_long_val(sim->m,argA,&argB);
        argA+=8;
        set_reg_val(sim->r,regA,argA);
        sim->pc = argB;
        break;
      case I_PUSHQ: /* A:0 regA:F */
        regB = (regid_t)REG_RSP;
        argA = get_reg_val(sim->r,regA);
        argB = get_reg_val(sim->r,regB);
        argB-=8;
        set_reg_val(sim->r,regB,argB);
        if (!set_long_val(sim->m,argB,argA)){
          err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,argB);
          return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
