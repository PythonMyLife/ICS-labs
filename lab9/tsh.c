...
void eval(char *cmdline) 
{
    char *argv[MAXARGS]; /* Argument list execue() */
...
void do_bgfg(char **argv) 
{
    ...
    else if(!strcmp(argv[0],"fg")){ /* fg command(wait until pid end) */
        job->state = FG;
        kill((job->pid * (-1)),SIGCONT);
        waitfg(job->pid);
    }
...
void waitfg(pid_t pid)
{
    while(pid == fgpid(jobs)){ /* loop until pid is not fg pid */
        sleep(0);
    }
    return;
}
